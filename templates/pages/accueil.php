    ...
    <main class="w300 mrlauto">
        ...
        <section id="commun">
            <div class="flex j-between mt40">
                ...
                <?php
                    if($espace === "vendeur"){
                        echo "<a href='index.php?controleur=afficher_historique&historique=vente'><button class='btnPad'>Historique des ventes</button></a>";
                    }else if($espace === "technicien"){
                        echo "<a href='index.php?controleur=afficher_historique&historique=ticketAll'><button class='btnPad'>Tickets archivés</button></a>";
                    }else if($espace === "client"){
                        echo "<a href='index.php?controleur=afficher_historique&historique=ticketPerso'><button class='btnPad'>Mes tickets</button></a>";
                    }
                ?>
            </div>
            <?php if($espace === "client") echo "<div class='flex j-end mt4'><a href='index.php?controleur=afficher_demande'>+ Nouveau ticket</a></div>"; ?>  
        </section>
        <div id="menu" class="encart d-none">
            <a href="index.php?controleur=deconnecter"><button class="btnPad">Deconnexion</button></a>
            <button id="closeMenu" class="btnPad">✖</button>
            <?php
                if($espace === "vendeur" || $espace === "technicien"){
                    echo "<a class='block mt16' href='index.php?controleur=afficher_modif_info'><button class='btnPad'>Modifier mon mot de passe</button></a>";
                }else if($espace === "client"){
                    echo "<a class='block mt16' href='index.php?controleur=afficher_modif_info'><button class='btnPad'>Modifier mes informations</button></a>";
                }
            ?>
        </div>  
        <!-- section spécifique -->
